...
// Mapping of image classes to C4C object category IDs, normally this would be a more sophisticated mechanism        
const labelMap = {
    "mice": "PS-MICE",
...
if (process.env.hasOwnProperty("APPGW_HOST")) {
    c4cClient.headers.Host = process.env.APPGW_HOST;
}
...
//Helper function to determine "neutral" file name  for image classifier
...
//Function that checks whether Service Request was created through e-mail
...
async function getEmailID(serviceRequestObjectID) {
    try {
        ...
        let result = response.data.d.results[0].ID;
        console.log(`Original E-Mail ID: ${result}`)
        return result
    } catch (error) {
        throw {
    ...
    try {
        ...
        for (i in response.data.d.results[0].EMailAttachments) {
            ...
            if (validMimeTypes.includes(attachment.MimeType.toLowerCase())) {
                ...
                console.log(`Found image ${attachment.Name} with mime ${attachment.MimeType}`)
            }
        }
        return result;
    } catch (error) {
...
async function getImageClassification(image, retry) {
    ...
    try {
        ...
        console.log(`Classification finished, identified the following Image classes: ${JSON.stringify(response.data.predictions[0].results)}`);
...
//Convert Leonardo Classification to C4C Category helper function
...
async function updateLabel(serviceRequestID, c4cLabel) {
    try {
        console.log(`Updating Service Request: ${serviceRequestID} with Classification value '${c4cLabel}'`)
